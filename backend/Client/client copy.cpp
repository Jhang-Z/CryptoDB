...
// 计算 FSS 结果，并以 JSON 格式返回 ans0 和 ans1
std::string compute_dcf_results(const std::string& input_str, const std::string& table) {
    uint64_t a = 0;
    try {
        a = std::stoull(input_str); // 转成 uint64_t
    } catch (...) {
    ...
    for (unsigned long i = 0; i < num_rows; ++i) {
        ...
        for (unsigned int col_idx = 0; col_idx < num_cols; ++col_idx) {
            ...
            lt_ans0 = evaluateLt(&fServer, &lt_k0, i_val);
            lt_ans1 = evaluateLt(&fServer, &lt_k1, i_val);

            column_results[col_name].first += lt_ans0 * col_val;
            column_results[col_name].second += lt_ans1 * col_val;
        }
    }
